...
namespace ITStockM.WebApi.Controllers
{
    ...
    public class GeminiChatController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendMessage([FromBody] ChatRequestModel request)
        {
            ...
            try
            {
                _logger.LogInformation("User sending message to Gemini: {MessagePreview}",
                ...
                var response = await _geminiChatService.SendMessageAsync(request.Message, _chatStateService.Messages);
            ...
            catch (InvalidOperationException ex)
            {
                ...
                return StatusCode(500, new { error = "Service is not properly configured.", details = ex.Message });
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(ex, "HTTP error calling Gemini API");
                return StatusCode(503, new { error = "Failed to reach Gemini API. Please try again later." });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error in Gemini chat");
                return StatusCode(500, new { error = "An unexpected error occurred. Please try again.", details = ex.Message });
            }
        }
        ...
        {
            return Ok(new { messageCount = _chatStateService.MessageCount });
        }
    }
