...
public class EmailBackgroundService : BackgroundService
{
    ...
    public EmailBackgroundService(IServiceProvider serviceProvider, EmailService emailService)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                var context = scope.ServiceProvider.GetRequiredService<ITStockManagmentService>();
                var assignments = (await context.GetAssignments(new Query { Expand = "AssignmentMateriels, AssignedEmployee, Employee" })).Where(a => a.OnMission && a.RestoreDate == null && a.RestoreDateLimit > DateTime.Today).ToList();
                var deliveryorders = (await context.GetDeliveryOrders()).Where(dlo => dlo.OrderNumber == null).ToList();
                var requests = (await context.GetRequests(new Query
                {
                    Filter = $@"i => i.Status == @0",
                    FilterParameters = new object[] { "Done" },
                    Expand = "Employee, Offers"
                })).Where(request => request.Offers != null && request.Offers.Any(offer => offer.Selected != null && offer.Selected == true && offer.DeliveryDate > DateTime.Today && offer.DeliveryDate <= DateTime.Today.AddDays(1))).ToList();

                if (!requests.IsNullOrEmpty())
                {
                    PendingDeliveries(requests);
                }


                if (!deliveryorders.IsNullOrEmpty())
                {
                    OrderNumbersNotFilled(deliveryorders);
                }
                if (!assignments.IsNullOrEmpty())
                {
                    ExpiredMissions(assignments);
                }
    ...
    protected void ExpiredMissions(List<Assignment> assignments)
    {
        ...
        foreach (var assignment in assignments)
        {
            ...
            {
                string materielList = string.Join(", ", assignment.AssignmentMateriels.Where(assm => assm.AssignmentId == assignment.Id).Select(m => m.Materiel.MaterielName));
...
<body>
    <div class=""container"">
        ...
        <div class=""content"">
            <p class=""greeting"">Hi {assignment.AssignedEmployee.FullName},</p>
            ...
            <div class=""materiel-list"">
                {string.Join("", assignment.AssignmentMateriels.Where(assm => assm.AssignmentId == assignment.Id).Select(m => $"<div class=\"materiel-item\">• {(m.Materiel.MaterielName)}</div>"))}
            </div>
        ...
        <div class=""footer"">
            ...
            <p class=""signature"">Best regards,<br>The AsteelFlash Team</p>
                ...
                // _emailService.SendEmail("sedkishabou7@gmail.com", "Final Reminder: Material Return Due Soon", htmlEmail);
...
<body>
    <div class=""container"">
        ...
        <div class=""content"">
            ...
            <p>This is a reminder that you have one week remaining to return these items. 
               We would appreciate it if you could bring them back by:</p>
            <div class=""due-date"">{assignment.RestoreDateLimit:MMMM dd, yyyy}</div>
            
            <p>Returning the material on time ensures that it's available for others who may need it.</p>
            
            <div class=""contact-info"">
                <p>If you've already returned the material, please disregard this email. If you anticipate needing 
                   a little more time or have any questions, please let me know as soon as possible at 
        ...
        <div class=""footer"">
            ...
            <p class=""signature"">Best regards,<br>The AsteelFlash Team</p>
               ...
               // _emailService.SendEmail("sedkishabou7@gmail.com", "Material Return Reminder", htmlEmail);
    ...
    protected void OrderNumbersNotFilled(List<DeliveryOrder> deliveryorders)
    {
    ...
    <div class=""container"">
        ...
        <div class=""content"">
            ...
            <div class=""order-list"">
                {string.Join("", deliveryorders.Select(dlo => $"<div class=\"order-item\">• Delivery Order: {dlo.DeleveryOrderNumber}</div>"))}
            </div>
       ...
       // _emailService.SendEmail("sedkishabou7@gmail.com", "Reminder: Unfilled Delivery Orders", htmlEmail);


    }

    protected void PendingDeliveries(List<Request> requests)
    {
        string htmlEmail = $@"<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class=""container"">
        ...
        <div class=""content"">
            <p class=""greeting"">Hello [Team/Recipient's Name],</p>
            ...
            <div class=""order-list"">
                ...
                <div class=""order-item"">
                    ...
                    <div class=""supplier"">
                        ...
                        {r.Offers.Where(o => o.Selected != null && o.Selected == true).FirstOrDefault().SupplierName}
                    </div>
                </div>"))}
            </div>
       ...
       // _emailService.SendEmail("sedkishabou7@gmail.com", "Reminder: Upcoming Delivery Orders – Arriving Tomorrow", htmlEmail);
